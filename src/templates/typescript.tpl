...
interface {{%name%}}Props {
    ...
    /**
     * The `ref` property represents a reference to the SVG element, allowing for direct manipulation or access to the DOM node.
     */
 ...
 * @param props.ref       A reference to the SVG element, allowing for direct manipulation or access to the DOM node.
 ...
 * ```
 */
export const {{%name%}} = ({className = '', {{%colors%}}height = {{%height%}}, ref, testId = '{{%name%}}', width = {{%width%}}}: {{%name%}}Props) => (
    <svg
        ref={ref}
        className={className}
        data-cy={testId}
